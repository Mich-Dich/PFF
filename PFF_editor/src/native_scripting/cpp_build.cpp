...
namespace PFF::cpp_build {
	...
	void build(const std::filesystem::path& project_directory) {
		...
		if (building_code || compiling) {
			...
			LOG(Warn, "building currently not posible => is currently building: " << util::to_string(building_code) << " / is currently compiling: " << util::to_string(compiling));
			return;
		}
	...
	void compile(const std::filesystem::path& project_directory) {
		...
		if (building_code || compiling) {
			...
			LOG(Warn, "compiling currently not posible => is currently building: " << util::to_string(building_code) << " / is currently compiling: " << util::to_string(compiling));
			return;
		}
