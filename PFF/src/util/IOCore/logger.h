...
PFF_API int log_init(const char* LogFileName, const char *directoryName, char* GeneralLogFormat, uintptr_t threadID, int Use_separate_Files_for_every_Thread);
PFF_API void log_output(const enum log_level level, const char* prefix, const char* funcName, char* fileName, const int Line, const uintptr_t thread_id, const char* message, ...);
...
#define LOG_Fatal(message, ...)                  { log_output(Fatal, "", __func__, __FILE__, __LINE__, CL_THREAD_ID, message, ##__VA_ARGS__); }
#define LOG_Error(message, ...)                  { log_output(Error, "", __func__, __FILE__, __LINE__, CL_THREAD_ID, message, ##__VA_ARGS__); }


// define conditional log macro for WARNINGS
#if LOG_LEVEL_ENABLED >= 1
    #define LOG_Warn(message, ...)               { log_output(Warn, "", __func__, __FILE__, __LINE__, CL_THREAD_ID, message, ##__VA_ARGS__); }
#else
    // Disabled by LogLevel
    #define LOG_Warn(message, ...)               {} while(0);
#endif

// define conditional log macro for INFO
#if LOG_LEVEL_ENABLED >= 2
    #define LOG_Info(message, ...)               { log_output(Info, "", __func__, __FILE__, __LINE__, CL_THREAD_ID, message, ##__VA_ARGS__); }
                 
#else
    // Disabled by LogLevel
    #define LOG_Info(message, ...)               {} while(0);
#endif

// define conditional log macro for DEBUG
#if LOG_LEVEL_ENABLED >= 3
    #define LOG_Debug(message, ...)              { log_output(Debug, "", __func__, __FILE__, __LINE__, CL_THREAD_ID, message, ##__VA_ARGS__); }
#else
    // Disabled by LogLevel
    #define LOG_Debug(message, ...)              {} while(0);
#endif

// define conditional log macro for TRACE
#if LOG_LEVEL_ENABLED >= 4
    #define LOG_Trace(message, ...)              { log_output(Trace, "", __func__, __FILE__, __LINE__, CL_THREAD_ID, message, ##__VA_ARGS__); }

    // Logs the start of a function, it would be helpful to has the '$F' in your format
    #define LOG_FUNC_START(message, ...)         { log_output(Trace, "START ", __func__, __FILE__, __LINE__, CL_THREAD_ID, message, ##__VA_ARGS__); }

    // Logs the end of a function, it would be helpful to has the '$F' in your format
    #define LOG_FUNC_END(message, ...)           { log_output(Trace, "END ", __func__, __FILE__, __LINE__, CL_THREAD_ID, message, ##__VA_ARGS__); }
    ...
    // Disabled by LogLevel
    #define LOG_FUNC_END(message, ...)           {} while(0);
    // Disabled by LogLevel
    #define LOG_FUNC_START(message, ...)         {} while(0);
    // Disabled by LogLevel
