...
class logged_exception : public std::exception {
	public: 
		explicit logged_exception(const char* file, const char* function , const int line, std::string&& message)
			: m_msg(message) { PFF::logger::log_msg(PFF::logger::severity::Error, file, function, line, std::move(message)); }
...
#define OG_LEVEL_ENABLED 					4


//  ===================================================================================  Logger calls  ===================================================================================

#define LOGGED_EXCEPTION(message)   { std::ostringstream log_oss{}; log_oss << "LOGGER EXCEPTION" << message; throw logged_exception(__FILE__, __FUNCTION__, __LINE__, std::move(log_oss.str())); }

#define LOG_Fatal(message)          { std::ostringstream log_oss{}; log_oss << message; PFF::logger::log_msg(PFF::logger::severity::Fatal, __FILE__, __FUNCTION__, __LINE__, std::move(log_oss.str())); }
#define LOG_Error(message)          { std::ostringstream log_oss{}; log_oss << message; PFF::logger::log_msg(PFF::logger::severity::Error, __FILE__, __FUNCTION__, __LINE__, std::move(log_oss.str())); }

#if LOG_LEVEL_ENABLED > 0
    #define LOG_Warn(message)       { std::ostringstream log_oss{}; log_oss << message; PFF::logger::log_msg(PFF::logger::severity::Warn, __FILE__, __FUNCTION__, __LINE__, std::move(log_oss.str())); }
#else
    #define LOG_Warn(message)       { }
#endif

#if LOG_LEVEL_ENABLED > 1
    #define LOG_Info(message)       { std::ostringstream log_oss{}; log_oss << message; PFF::logger::log_msg(PFF::logger::severity::Info, __FILE__, __FUNCTION__, __LINE__, std::move(log_oss.str())); }
#else
    #define LOG_Info(message)       { }
#endif

#if LOG_LEVEL_ENABLED > 2
    #define LOG_Debug(message)      { std::ostringstream log_oss{}; log_oss << message; PFF::logger::log_msg(PFF::logger::severity::Debug, __FILE__, __FUNCTION__, __LINE__, std::move(log_oss.str())); }
#else
    #define LOG_Debug(message)      { }
#endif

#if LOG_LEVEL_ENABLED > 3
    #define LOG_Trace(message)      { std::ostringstream log_oss{}; log_oss << message; PFF::logger::log_msg(PFF::logger::severity::Trace, __FILE__, __FUNCTION__, __LINE__, std::move(log_oss.str())); }
#else
