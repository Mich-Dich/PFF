...
namespace PFF {
	...
	procedural_mesh_script::procedural_mesh_script() {
		...
		m_mesh_asset = create_ref<PFF::geometry::mesh_asset>();
	}
	...
	void procedural_mesh_script::apply_mesh() {

		m_mesh_asset->calc_bounds();
		m_mesh_asset->mesh_buffers = GET_RENDERER.upload_mesh(m_mesh_asset->indices, m_mesh_asset->vertices);
	}
